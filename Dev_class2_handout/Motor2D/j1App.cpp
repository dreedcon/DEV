...
bool j1App::DocLoad()
{
	...
	pugi::xml_node     node;
	pugi::xml_parse_result result = doc.load_file("save_load.xml");

	if (result == NULL)
		ret = false;
	else
	{
		LOG("Acces to the Doc");
		node = doc.child("configsaveload");
		...
		while (item != NULL && ret == true)
		{
			//This is the same to the awake method
...
bool j1App::DocSave() const
{
	...
	else
	{
		...
		{
			//This is the same to the awake method
			ret = item->data->Save(config.child(item->data->name.GetString()));
			item = item->next;
		}
