...
bool j1Render::Load(pugi::xml_node &node)
{
	bool ret = true;
	 ...
	 camera.x = node.child("loadpos").attribute("posX").as_int();
	 camera.y = node.child("loadpos").attribute("posY").as_int();
...
bool j1Render::Save(pugi::xml_node &node)
{
	...
	camera.x = node.append_child("loadpos").append_attribute("posX").as_int();
	camera.y = node.append_child("loadpos").append_attribute("posY").as_int();
